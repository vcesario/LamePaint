...
namespace LameUI
{
	...
	bool OpenFile(std::string& filePath) // thanks https://stackoverflow.com/a/72429080
	{
	...
	bool AskSavePath(std::string& filePath)
	{
		...
		f_FileSystem->SetFileName(L"image.png");
	...
	void DrawBottomBar(int posX, int posY, float fps)
	{
		...
		if (ImGui::BeginViewportSideBar("StatusBar", viewport, ImGuiDir_Down, height, window_flags))
		{
			...
			{
				if (!m_CursorOnUi && LameInput::IsCursorInsideWindow())
				{
	...
	void UpdateCursorVisibility(GLFWwindow* window)
	{
		if (m_CursorOnUi)
		{
	...
	void Render(GLFWwindow* window, int pixelX, int pixelY, float framerate, TextureObject iconTex)
	{
		// imgui logic
